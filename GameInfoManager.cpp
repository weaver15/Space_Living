...
void GameInfoManager::LoadInventoryInformation(Player* player)
{
	pugi::xml_node inventory = Doc.child("Inventory");
	SDL_Rect inventoryTextureInformation = {
		inventory.child("Texture").attribute("x").as_int(),
		inventory.child("Texture").attribute("y").as_int(),
		inventory.child("Texture").attribute("width").as_int(),
		inventory.child("Texture").attribute("height").as_int()
	};
	SDL_Rect selectorTextureInformation = {
		inventory.child("SelectorTexture").attribute("x").as_int(),
		inventory.child("SelectorTexture").attribute("y").as_int(),
		inventory.child("SelectorTexture").attribute("width").as_int(),
		inventory.child("SelectorTexture").attribute("height").as_int()
	};
	pugi::xml_node equipmentInventory = inventory.child("EquipTexture");
	SDL_Rect equipmentTextureInformation = {
		equipmentInventory.attribute("x").as_int(),
		equipmentInventory.attribute("y").as_int(),
		equipmentInventory.attribute("width").as_int(),
		equipmentInventory.attribute("height").as_int()
	};

	SDL_Point equipmentHeadSlot = { equipmentInventory.child("headSlot").attribute("x").as_int(),
		equipmentInventory.child("headSlot").attribute("y").as_int() };
	SDL_Point equipmentChestSlot = { equipmentInventory.child("chestSlot").attribute("x").as_int(),
		equipmentInventory.child("chestSlot").attribute("y").as_int() };
	SDL_Point equipmentLegsSlot = { equipmentInventory.child("legsSlot").attribute("x").as_int(),
		equipmentInventory.child("legsSlot").attribute("y").as_int() };
	SDL_Point equipmentHandSlot = { equipmentInventory.child("handSlot").attribute("x").as_int(),
		equipmentInventory.child("handSlot").attribute("y").as_int() };
	SDL_Point equipmentWeapon1Slot = { equipmentInventory.child("weapon1Slot").attribute("x").as_int(),
		equipmentInventory.child("weapon1Slot").attribute("y").as_int() };
	SDL_Point equipmentWeapon2Slot = { equipmentInventory.child("weapon2Slot").attribute("x").as_int(),
		equipmentInventory.child("weapon2Slot").attribute("y").as_int() };


	InventoryInformation invInfo;
	pugi::xml_node infoBox = inventory.child("InformationBox");
	invInfo.TextBoxTextureInfo = {
		infoBox.attribute("x").as_int(),
		infoBox.attribute("y").as_int(),
		infoBox.attribute("width").as_int(),
		infoBox.attribute("height").as_int()
	};


	invInfo.TextureInfo = inventoryTextureInformation;
	invInfo.SelectorTextureInfo = selectorTextureInformation;
	invInfo.EquipmentTextureInfo = equipmentTextureInformation;

	EquipmentSlotInformation eqpInfo;
	eqpInfo.HeadSlot = equipmentHeadSlot;
	eqpInfo.ChestSlot = equipmentChestSlot;
	eqpInfo.LegsSlot = equipmentLegsSlot;
	eqpInfo.HandSlot = equipmentHandSlot;
	eqpInfo.Weapon1Slot = equipmentWeapon1Slot;
	eqpInfo.Weapon2Slot = equipmentWeapon2Slot;

	player->InitializeInventory(invInfo, eqpInfo);
}

void GameInfoManager::LoadLootTableInformation(LootTable* lootTable)
{
	pugi::xml_node inventory = Doc.child("Inventory");
	*(lootTable) = LootTable();
	SDL_Rect ltLocation = { 0, 0, 34, 34 };
	SDL_Point ltRowCol = { 1,1 };
	pugi::xml_node itemsTexture = inventory.child("ItemsTexture");
	int size = itemsTexture.attribute("width").as_int();

	pugi::xml_node pistol = itemsTexture.child("pistol");
	SDL_Rect pistolTexture = { pistol.attribute("x").as_int(), pistol.attribute("y").as_int(), size , size };
	InventoryItem pistolLootTableItem = InventoryItem(pistolTexture, ltLocation, ltRowCol);
	pistolLootTableItem.SetInventoryType(InventoryType::Weapon);
	pistolLootTableItem.SetWeaponType(WeaponType::Pistol);
	lootTable->AddItem(pistolLootTableItem);

	pugi::xml_node chest = itemsTexture.child("chest");
	SDL_Rect chestTexture = { chest.attribute("x").as_int(), chest.attribute("y").as_int(), size , size };
	InventoryItem chestlootTableItem = InventoryItem(chestTexture, ltLocation, ltRowCol);
	chestlootTableItem.SetInventoryType(InventoryType::Armor);
	chestlootTableItem.SetArmorType(ArmorType::Chest);
	lootTable->AddItem(chestlootTableItem);

	pugi::xml_node head = itemsTexture.child("head");
	SDL_Rect headTexture = { head.attribute("x").as_int(), head.attribute("y").as_int(), size , size };
	InventoryItem headLootTableItem = InventoryItem(headTexture, ltLocation, ltRowCol);
	headLootTableItem.SetInventoryType(InventoryType::Armor);
	headLootTableItem.SetArmorType(ArmorType::Head);
	lootTable->AddItem(headLootTableItem);

	pugi::xml_node rifle = itemsTexture.child("rifle");
	SDL_Rect rifleTexture = { rifle.attribute("x").as_int(), rifle.attribute("y").as_int(), size , size };
	InventoryItem rifleLootTableItem = InventoryItem(rifleTexture, ltLocation, ltRowCol);
	rifleLootTableItem.SetInventoryType(InventoryType::Weapon);
	rifleLootTableItem.SetWeaponType(WeaponType::Rifle);
	lootTable->AddItem(rifleLootTableItem);
}
...
void GameInfoManager::LoadItemInformation(DroppedObjectManager* manager)
{
	pugi::xml_node item = Doc.child("Item");
	int maxNumberOfDroppedObjects = item.attribute("max_dropped_items_on_screen").as_int();
	*(manager) = DroppedObjectManager(maxNumberOfDroppedObjects);

	SDL_Rect textureInfo{
		item.attribute("x").as_int(),
		item.attribute("y").as_int(),
		item.attribute("width").as_int(),
		item.attribute("height").as_int()
	};
	manager->SetTextureLocation(textureInfo);

	pugi::xml_node key = Level.child("Keys");
	int width = key.child("Texture").attribute("width").as_int();
	int height = key.child("Texture").attribute("height").as_int();
	SDL_Rect keyTextureInfo{
		key.child("Texture").attribute("x").as_int(),
		key.child("Texture").attribute("y").as_int(),
		width,
		height
	};
	SDL_Rect location
	{
		key.child("Location").attribute("x").as_int(),
		key.child("Location").attribute("y").as_int(),
		width,
		height
	};
	auto bossKey = DroppedObject(keyTextureInfo, location, DroppedObjectType::Key);
	manager->AddDroppedObject(bossKey);
}
